 ...
 *
 * The rule: **dev and staging tell crawlers to stay out entirely; production
 * does not.** Both hostnames are publicly resolvable with real certificates,
 * so crawlers reach them — Certificate Transparency logs are routinely
 * scraped for hostnames. `<meta name="robots" content="noindex">` in the root
 * layout keeps those pages out of search results, but it only works on HTML a
 * crawler has already fetched: it does nothing about the crawl traffic
 * itself, and nothing at all for non-HTML paths like the raw `/src/*.ts`
 * modules Vite serves in dev.
 *
 * The same flag drives both, so a site cannot end up half-hidden:
 * VITE_NOINDEX is what the layout's meta tag reads too (see $lib/noindex).
 *
 ...
 * production the next time it shipped. Getting that backwards is expensive
 * and silent, which is what the production case below is guarding.
 */
...
async function robotsFor(noindex: boolean) {
	vi.resetModules();
	vi.doMock('$lib/noindex', () => ({ NOINDEX: noindex }));
...
describe('robots.txt on dev and staging (VITE_NOINDEX=true)', () => {
...
describe('robots.txt on production (VITE_NOINDEX=false)', () => {
